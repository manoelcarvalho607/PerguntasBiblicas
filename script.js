...
function perguntaSeguinte() {
    ...
    // adiciona as respostas em botões de forma aleatória
    const { correta, erradas } = lista[numeroPerguntaAtual];
    const respostasAleatorias = [correta, ...erradas].sort(() => Math.random() - 0.5);

    c('.bt_1').innerHTML = respostasAleatorias[0];
    c('.bt_2').innerHTML = respostasAleatorias[1];
    c('.bt_3').innerHTML = respostasAleatorias[2];
    c('.bt_4').innerHTML = respostasAleatorias[3];
...
function botao(numeroDoBotao) {
    const { correta } = lista[numeroPerguntaAtual];
    const escolhida = c(`.bt_${numeroDoBotao}`).innerHTML;
